...
public class EmploymentViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
    ...
    public EmploymentViewHolder(@NonNull View itemView, OnDashboardListener mOnListener, RequestManager requestManager, ViewPreloadSizeProvider<String> preloadSizeProvider) {
        super(itemView);
    ...
    public void onBind(Employment mList, Context mContext, int i, String user_id) {
        ...
        if (mList.getPrice().isEmpty())
            price.setVisibility(View.GONE);
        else
            price.setText(String.format("₹ %1$s", mList.getPrice()));

        String emp_status = (mList.getEmp_status().equals("1")) ? mContext.getString(R.string.kam_pahije) : mContext.getString(R.string.roj_pahije);
    ...
    private void createGallery(String[] images, RecyclerView rvImages, Employment productArrayList, Context mContext) {
