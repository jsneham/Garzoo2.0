...
public class PromotionViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
    ...
    public PromotionViewHolder(@NonNull View itemView, OnDashboardListener mOnListener, RequestManager requestManager, ViewPreloadSizeProvider<String> preloadSizeProvider) {
        super(itemView);
    ...
    public void onBind(Promotion mList, Context mContext, int i, String user_id) {
        ...
        if (mList.getUser_id().equals(user_id)) {
            ...
            // viewHolder.ivChat.setCompoundDrawablesWithIntrinsicBounds(R.drawable.ic_message_own, 0, 0, 0);
            Glide.with(mContext).load(R.drawable.ic_baseline_call_hide).into(ivCall);
        } else {
        ...
        String emp_status = (mList.getPd_status().equals("1")) ? mContext.getString(R.string.jahirat_) : mContext.getString(R.string.charcha_);
    ...
    private void createGallery(String[] images, RecyclerView rvImages, Promotion productArrayList, Context mContext) {
