...
public class RentViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
    ...
    public void onBind(Rent mRentList, Context mContext, int i , String user_id) {
        if (mRentList.getMobile_status().equals("1"))
            Glide.with(mContext).load(R.drawable.ic_baseline_call_hide).into(ivCall);
        if (mRentList.getUser_id().equals(user_id)) {
        ...
        if (mRentList.getVideo() == null || mRentList.getVideo().equals("")) {
            flVideo.setVisibility(View.GONE);
        } else {
            flVideo.setVisibility(View.VISIBLE);
            //Picasso.get().load(URLs.IMAGE_URL + mRentList.getImage()).into(image);
            Glide.with(mContext)
                    .load(mRentList.getVideo()).into(image);
        }
