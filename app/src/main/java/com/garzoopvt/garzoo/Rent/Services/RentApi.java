...
public interface RentApi {
    ...
    // GET Rent REQUEST
    @GET(URLs.api_rent_data_2_0)
    LiveData<ApiResponse<RentResponse>> getRent(
    ...
    @GET(URLs.api_rent_category_data_2_0)
    LiveData<ApiResponse<CategoryResponse>> getCategory(
            @Query("unique_id") String unique_id,
            @Query("language_code") String language_code
    );


    @Multipart
    @POST(URLs.api_rent_add_record)
    Call<ResponseBody> upload(
            ...
            @Part("listing_status") RequestBody  listing_status,
            @Part("available_status") RequestBody  available_status
    ...
    @Multipart
    @POST(URLs.api_add_listing_interest_2_0)
    LiveData<ApiResponse<RentResponse>> interest1(
            @Part("unique_id") RequestBody unique_id,
            @Part("user_id") RequestBody user_id,
            @Part("to_user_id") RequestBody  to_user_id,
            @Part("full_name") RequestBody  full_name,
    ...
    @Multipart
    @POST(URLs.api_rent_edit_record_2_0)
    ...
    LiveData<ApiResponse<RentResponse>> Listing_rent_delete_record_2_0(
            @Query("unique_id") String unique_id,
            @Query("p") String post_id);


    @GET(URLs.api_rent_available)
